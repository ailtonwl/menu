// src/components/venda/VendaFiltro.tsx
import React from 'react';

type Props = {
  pessoas: { id: number; nome: string }[];
  filtros: { dataInicio: string; dataFim: string; pessoaId: number; status: string[]; ativo: boolean};
  setFiltros: (f: any) => void;
  aplicarFiltros: () => void;
  desfazerFiltros: () => void;
};

export default function VendaFiltro({ pessoas, filtros, setFiltros, aplicarFiltros, desfazerFiltros }: Props) {
  return (
    <div className="filtro-container">
      ...
      <div className="ckBoxes">
        <div className="itemBox">
          <label>Status:</label>
          {['Aberto', 'Recebido', 'Parcial'].map(status => (
            <label key={status}>
              <input
                ...
                onChange={(e) => {
                  ...
                  setFiltros({ ...filtros, status: novoStatus });
                }}
              />
              {status}
            </label>
